...
/**
 * Laucher for Java classes. Allows to modify the classpath at runtime instead of relying on the jar-mechanism.
 *
...
public class Launcher
{
  private static class JarFilter implements FileFilter
  {
    ...
    public boolean accept(final File pathname)
    {
      ...
      return
          (base.regionMatches(true, base.length() - JAR_SUFFIX.length(), JAR_SUFFIX, 0, JAR_SUFFIX.length()) ||
              base.regionMatches(true, base.length() - ZIP_SUFFIX.length(), ZIP_SUFFIX, 0, ZIP_SUFFIX.length()));
    }
  }
  ...
  private static void addLibrary(final File directory, final String path, final ArrayList list)
  {
  ...
  private static void parseConfiguration(final File configuration)
  {
    try
    {
      ...
      final InputStream in = new BufferedInputStream(new FileInputStream(configuration));
      try
      {
        p.load(in);
      }
  ...
  private static void parsePath(final String path, final ArrayList list, final String separator)
  {
    if (path == null)
    {
      return;
  ...
  private static File computeApplicationDir(final URL location, final File defaultDir)
  {
    ...
    {
      System.err.println
          ("Warning: Unrecognized location type. Assuming default.");
      return new File(".");
    }
